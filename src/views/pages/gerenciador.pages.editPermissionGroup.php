    ...
    <?php $render('gerenciador.partials.aside',['loggedUser'=>$loggedUser]);?>
        ...
        <section>
            ...
            <div class="content colum " >               
                ...
                <form method="Post" action="<?=$base?>/editPermissionGroupAction/<?=$idGroup?>">
               
                    <div class="campo <?=(!empty($flash))? 'has-error':''?>">
                        <label class="form-lbl" for="name">Nome:</label>
                        <input class="form-cmps" type="text" name="name" value="<?=$name['name']?>" id="name">
                    </div>
             
                    <?php foreach($items as $item):?>
                        <div class="campo">
                            <input <?=(in_array($item['slug'],$permissionLinks))?'checked':''?> class="checkBox" type="checkbox" name="itemPermission[]" value="<?=$item['id']?>" id="item-<?=$item['id']?>">
